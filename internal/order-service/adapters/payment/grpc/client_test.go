...
type scriptedPaymentServer struct {
	paymentpb.UnimplementedPaymentServiceServer
	calls atomic.Int32
}

func (s *scriptedPaymentServer) Charge(_ context.Context, _ *paymentpb.ChargeRequest) (*paymentpb.ChargeResponse, error) {
	if s.calls.Add(1) <= 6 {
		return &paymentpb.ChargeResponse{Status: paymentpb.PaymentStatus_FAILED, Message: "insufficient funds"}, nil
	}
	return &paymentpb.ChargeResponse{Status: paymentpb.PaymentStatus_SUCCEEDED, TransactionId: "tx-ok"}, nil
}

func TestPaymentClient_Charge_businessFailureDoesNotOpenBreaker(t *testing.T) {
	t.Parallel()
	...
	srv := grpcserver.NewServer()
	paymentpb.RegisterPaymentServiceServer(srv, &scriptedPaymentServer{})
	go func() { _ = srv.Serve(lis) }()
	t.Cleanup(srv.GracefulStop)
	...
	for range 6 {
		_, ok, _, err := client.Charge(context.Background(), "order-1", "user-1", 1000)
		require.NoError(t, err)
		assert.False(t, ok)
	}

	txID, ok, _, err := client.Charge(context.Background(), "order-1", "user-1", 1000)
	require.NoError(t, err)
	assert.True(t, ok)
